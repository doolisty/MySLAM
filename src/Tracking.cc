...
Tracking::Tracking(
    ...
    : mSensor(sensor),
      ...
      mpORBVocabulary(pVoc) /*, mpPointCloudMapping( pPointCloud )*/,
      mpKeyFrameDB(pKFDB),
  ...
  if (sensor == System::STEREO || sensor == System::RGBD) {
    mThDepth =
        Camera::bf * (float)fSettings["ThDepth"] / Camera::K.at<float>(0, 0);
  ...
  if (mSensor != System::MONOCULAR) {
    for (int i = 0; i < mCurrentFrame.N; i++) {
      if (mCurrentFrame.mvDepth[i] > 0 && mCurrentFrame.mvDepth[i] < mThDepth) {
        nTotal++;
        if (mCurrentFrame.mvpMapPoints[i])
          if (mCurrentFrame.mvpMapPoints[i]->Observations() > 0) nMap++;
      }
    }
  } else {
...
void Tracking::CreateNewKeyFrame() {
  ...
  // mpPointCloudMapping->insertKeyFrame( pKF,this->mImS_C,this->mImS
