  ...
  if (flag_orb_mov == 1) {
    for (int level = 0; level < nlevels; ++level) {
      ...
      while (keypoint != mkeypoints.end()) {
        ...
        int label_coord =
            (int)imS.ptr<uchar>((int)search_coord.y)[(int)search_coord.x];
...
void ORBextractor::ProcessDesp(cv::InputArray _image, cv::InputArray _mask,
                               vector<vector<cv::KeyPoint>>& _allKeypoints,
                               vector<cv::KeyPoint>& _mKeypoints,
  ...
  for (int level = 0; level < nlevels; ++level) {
    ...
    GaussianBlur(workingMat, workingMat, cv::Size(7, 7), 2, 2,
