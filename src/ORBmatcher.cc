...
int ORBmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw,
  ...
  for (int iMP = 0; iMP < nPoints; iMP++) {
    ...
    const int nPredictedLevel =
        pMP->PredictScale(dist3D, pKF->mfLogScaleFactor);
...
int ORBmatcher::SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2,
  ...
  for (int i1 = 0; i1 < N1; i1++) {
    ...
    const int nPredictedLevel =
        pMP->PredictScale(dist3D, pKF2->mfLogScaleFactor);
  ...
  for (int i2 = 0; i2 < N2; i2++) {
    ...
    // Compute predicted octave
    const int nPredictedLevel =
        pMP->PredictScale(dist3D, pKF1->mfLogScaleFactor);
  ...
  for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
    ...
    if (pMP) {
      if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
        ...
        int nPredictedLevel =
            pMP->PredictScale(dist3D, CurrentFrame.mfLogScaleFactor);
