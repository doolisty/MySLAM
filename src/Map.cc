...
KeyFrame *Map::_ReadKeyFrame(ifstream &f, ORBVocabulary &voc,
...
bool Map::Load(const string &filename, ORBVocabulary &voc) {
  if (!Camera::initialized) {
    cerr << "Map: camera is not initialized. Cowardly refusing to load anything"
         << endl;
    return false;
  }
  ...
  ORB_SLAM2::ORBextractor orb_ext = ORB_SLAM2::ORBextractor(
      nFeatures, scaleFactor, nLevels, fIniThFAST, fMinThFAST);
  ...
  for (auto kf : mspKeyFrames) kf_by_id[kf->mnId] = kf;

  for (auto kf : kf_by_order) {
    unsigned long int parent_id;
    f.read((char *)&parent_id, sizeof(parent_id));
    if (parent_id != ULONG_MAX) kf->ChangeParent(kf_by_id[parent_id]);
...
bool Map::Save(const string &filename) {
  ...
  for (auto mp : mspMapPoints) _WriteMapPoint(f, mp);
  ...
  for (auto kf : mspKeyFrames) _WriteKeyFrame(f, kf, idx_of_mp);
