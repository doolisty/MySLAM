...
ORB_SLAM2,
 * we add anther two threads including semantic segmentation thread and densemap
creation thread.
 * You should pay attention to Frame.cc, ORBmatcher.cc, Pointcloudmapping.cc and
Segment.cc.
 *
 *　@article{murORB2,
 *　title={{ORB-SLAM2}: an Open-Source {SLAM} System for Monocular, Stereo and
{RGB-D} Cameras}, 　*　author={Mur-Artal, Ra\'ul and Tard\'os, Juan D.}, 　*
journal={IEEE Transactions on Robotics}, 　*　volume={33}, 　* number={5}, 　*
pages={1255--1262}, 　* doi = {10.1109/TRO.2017.2705103}, 　* year={2017} *　}
 * --------------------------------------------------------------------------------------------------
...
void ComputeImageBounds() {
  if (Camera::DistCoef.at<float>(0) != 0.0) {
    ...
    // Undistort corners
    mat = mat.reshape(2);
    cv::undistortPoints(mat, mat, Camera::K, Camera::DistCoef, cv::Mat(),
                        Camera::K);
    mat = mat.reshape(1);
...
void Frame::InitializeClass() {
  if (Frame::mbInitialComputations) {
    ...
    Frame::mfGridElementWidthInv =
        static_cast<float>(FRAME_GRID_COLS) / (Frame::mnMaxX - Frame::mnMinX);
    Frame::mfGridElementHeightInv =
        static_cast<float>(FRAME_GRID_ROWS) / (Frame::mnMaxY - Frame::mnMinY);
    Frame::mbInitialComputations = false;
  }
}

Frame::Frame() { category_stat_ptr_ = new std::unordered_map<int, PtStat>(); }
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth,
             ...
             ORBVocabulary *voc, const float &thDepth)
    : mpORBvocabulary(voc),
  ...
  orbExtractTime =
      std::chrono::duration_cast<std::chrono::duration<double>>(t22 - t11)
          .count();
  ...
  if (imGrayPre.data) {
    ...
    std::chrono::steady_clock::time_point tm2 =
        std::chrono::steady_clock::now();
    movingDetectTime =
        std::chrono::duration_cast<std::chrono::duration<double>>(tm2 - tm1)
            .count();
...
void Frame::ExtractORBDesp(const cv::Mat &imgray) {
  (*mpORBextractorLeft)
      .ProcessDesp(imgray, cv::Mat(), mvKeysTemp, mvKeys, mDescriptors);
}
...
void Frame::ProcessMovingObject(const cv::Mat &imgray) {
  ...
  cv::goodFeaturesToTrack(imGrayPre, prepoint, 1000, 0.01, 8, cv::Mat(), 3,
                          true, 0.04);
  cv::cornerSubPix(
      imGrayPre, prepoint, cv::Size(10, 10), cv::Size(-1, -1),
      cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.03));
  cv::calcOpticalFlowPyrLK(
      imGrayPre, imgray, prepoint, nextpoint, state, err, cv::Size(22, 22), 5,
  ...
  for (int i = 0; i < state.size(); i++) {
    if (state[i] != 0) {
      ...
      if ((x1 < limit_edge_corner || x1 >= imgray.cols - limit_edge_corner ||
           x2 < limit_edge_corner || x2 >= imgray.cols - limit_edge_corner ||
           y1 < limit_edge_corner || y1 >= imgray.rows - limit_edge_corner ||
           y2 < limit_edge_corner || y2 >= imgray.rows - limit_edge_corner)) {
        state[i] = 0;
        continue;
      }
      double sum_check = 0;
      for (int j = 0; j < 9; j++) {
        sum_check += abs(imGrayPre.at<uchar>(y1 + dy[j], x1 + dx[j]) -
                         imgray.at<uchar>(y2 + dy[j], x2 + dx[j]));
      }
  ...
  cv::Mat F = cv::findFundamentalMat(F_prepoint, F_nextpoint, mask,
  ...
  for (int i = 0; i < mask.rows; i++) {
    if (mask.at<uchar>(i, 0) == 0) {
      ...
      double A = F.at<double>(0, 0) * F_prepoint[i].x +
                 F.at<double>(0, 1) * F_prepoint[i].y + F.at<double>(0, 2);
      double B = F.at<double>(1, 0) * F_prepoint[i].x +
                 F.at<double>(1, 1) * F_prepoint[i].y + F.at<double>(1, 2);
      double C = F.at<double>(2, 0) * F_prepoint[i].x +
                 F.at<double>(2, 1) * F_prepoint[i].y + F.at<double>(2, 2);
      double dd = fabs(A * F_nextpoint[i].x + B * F_nextpoint[i].y + C) /
                  sqrt(A * A + B * B);  // Epipolar constraints
      if (dd <= 0.1) {
  ...
  for (int i = 0; i < prepoint.size(); i++) {
    if (state[i] != 0) {
      ...
      double dd = fabs(A * nextpoint[i].x + B * nextpoint[i].y + C) /
                  sqrt(A * A + B * B);
...
void Frame::ProcessMovingObjectSeg(const cv::Mat &imgray,
  ...
  auto update_formula = [this](const int &n_dy, const int &n_st,
