...
System::System(const string &strVocFile, const string &strSettingsFile,
  ...
  cout << "[LocalMapping] running in thread " << hex
       << mptLocalMapping->get_id() << endl;

  // Initialize the Loop Closing thread and launch
  mpLoopCloser = new LoopClosing(mpMap, mpKeyFrameDatabase, mpVocabulary,
                                 mSensor != MONOCULAR);
  mptLoopClosing = new thread(&ORB_SLAM2::LoopClosing::Run, mpLoopCloser);
  cout << "[LoopClosing] running in thread " << hex << mptLoopClosing->get_id()
       << endl;
...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap,
                          const cv::Mat &segimg, const double &timestamp) {
  if (mSensor != RGBD) {
...
int System::GetStatus() { return mpTracker->mState; }
